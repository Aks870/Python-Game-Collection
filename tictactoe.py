...
def drawgrid(): #drawing the board with lines
    ...
    pygame.draw.line (screen, (236, 248, 127), ((400/3) * 2, 0), ((400/3) * 2, 400), 6)#the second vertical line
    #are multiplied by 2
...
def result(): #to get results and print them on screen
    ...
    font = pygame.font.SysFont('BAZOOKA', 70)
...
def wincases(): # check the winner and draw a line across how the win was
    ...
    for row in range(0, 3):
        if((board[row][0] == board[row][1] == board [row][2]) and (board [row][0] != None)):
            winner = board[row] [0]
            pygame.draw.line(screen, (250, 0, 0), (0, (row + 1)*400 / 3 -400 / 6), (400, (row + 1)*400 / 3 -400 / 6),)
            result()
            break
    ...
    for col in range(0, 3):
        if((board[0][col] == board[1] [col] == board[2][col]) and (board[0][col] != None)):
            winner = board[0][col]
            pygame.draw.line(screen, (250, 0, 0), ((col+1)* 400 / 3 - 400 / 6, 0),((col + 1)* 400 / 3 -400 / 6,400))
            result()
            break
...
def input_to_block(): #to get the position of the coordinate clicked so that you use the above function draw to get
